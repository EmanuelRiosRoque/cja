...
>
  ...
  <div class="relative">
    ...
    <button type="button"
      ...
      <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
              d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 0 0 2-2V7a2 2 0 0 0-2-2H5a 2 2 0 0 0-2 2v12a 2 2 0 0 0 2 2z"/>
      </svg>
    </button>
    ...
    <input
      ...
      {{-- 🔥 Aquí jalamos el valor inicial de Livewire (ej. fecha_recepcion) --}}
      x-init="
        if ($wire && $wire.get('{{ $wireModelAttr }}')) {
          value = $wire.get('{{ $wireModelAttr }}');
    ...
    <div x-show="isOpen" x-transition x-cloak
         ...
         wire:ignore
         x-ref="panel"
         class="absolute z-[9999] mt-1 top-full left-0 w-[20rem] bg-white rounded-2xl shadow-xl ring-1 ring-neutral-200">
        ...
        <div class="flex items-center justify-between">
            ...
            <button type="button" @click="toggleYearList()"
              ...
              <svg class="w-4 h-4 text-neutral-500" viewBox="0 0 20 20" fill="currentColor">
                <path fill-rule="evenodd"
                  d="M5.23 7.21a.75.75 0 011.06.02L10 11.11l3.71-3.88a.75.75 0 011.08 1.04l-4.25 4.45a.75.75 0 01-1.08 0L5.21 8.27a.75.75 0 01.02-1.06z"
                  clip-rule="evenodd"/>
              </svg>
            </button>
          ...
          <div class="flex items-center gap-2">
            <button @click="prevMonth()" class="p-1 rounded hover:bg-neutral-100" aria-label="Mes anterior">
              <svg class="w-5 h-5 text-neutral-600" viewBox="0 0 24 24" fill="none" stroke="currentColor">
      ...
      <div class="grid grid-cols-7 gap-1 px-3 text-[11px] font-medium text-neutral-500">
        <template x-for="d in weekdaysOrdered" :key="d">
...
document.addEventListener('alpine:init', () => {
  Alpine.data('dpLivewire', (opts = {}) => ({
    ...
    get weekdaysOrdered() {
      return [...this.weekdays.slice(this.firstDay), ...this.weekdays.slice(0, this.firstDay)];
    },

    years: (() => {
      const now = new Date().getFullYear(), s = now - 80, e = now + 20;
    ...
    toggleYearList() {
      ...
      if (this.showYears) this.showMonths = false;
    },
    toggleMonthList() {
      this.showMonths = !this.showMonths;
      if (this.showMonths) this.showYears = false;
    },
    selectYear(y) {
      this.year = y;
      this.showYears = false;
    ...
    isDisabled(day) {
      return this.isDateDisabled(this.isoOf(day));
    },
